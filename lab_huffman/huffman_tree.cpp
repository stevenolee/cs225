...
// if neither are NULL
	TreeNode* test1 = singleQueue.front();
	TreeNode* test2 = mergeQueue.front();
	if (test1->freq.getFrequency() <= test2->freq.getFrequency()){
		singleQueue.pop();
		return test1;
	} else {
		mergeQueue.pop();
...
// add all frequencies into singleQueue
	for (auto& it : frequencies){
		TreeNode* element = new TreeNode(it);
	...
	while ( (singleQueue.size() + mergeQueue.size()) > 1){
			...
			int newFrequency = first->freq.getFrequency() + second->freq.getFrequency();
			TreeNode* newHead = new TreeNode (newFrequency);
			newHead->left = first;
			newHead->right = second;
			mergeQueue.push(newHead);
	}

	if (singleQueue.size() == 1){
		TreeNode* front = singleQueue.front();
		root_ = front;
		singleQueue.pop();
	} else if (mergeQueue.size() == 1) {
		TreeNode* front = mergeQueue.front();
		root_ = front;
		mergeQueue.pop();
	}
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
	...
	if (current->left == NULL && current->right == NULL){
		bfile.writeBit(1);
		char data = current->freq.getCharacter();
		bfile.writeByte(data);
	} else {
		bfile.writeBit(0);
